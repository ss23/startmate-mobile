...
class _SetPageState extends State<SetPage> {
  ...
  Future<void> getSetData() async {
    ...
    // TODO: Pagination
    var query =
        r'query GetGames($phaseGroupId: ID!, $page: Int) { phaseGroup(id: $phaseGroupId) { sets(page: $page) { pageInfo { totalPages } nodes { id fullRoundText round identifier totalGames winnerId slots { entrant { id name } } games { winnerId } } } } }';
  ...
  Widget build(BuildContext context) {
    ...
    Map<int, List<GGSet>> setsByRound = {};
    int finalSet = 0;

    for (var set in sets!) {
      if (setsByRound[set.round] == null) {
        setsByRound[set.round!] = [];
      }
      setsByRound[set.round]!.add(set);
      if (set.round! > finalSet) {
        finalSet = set.round!;
      }
    }

    print("Running up to largest set of ${finalSet}");

    return ListView(
      scrollDirection: Axis.horizontal,
      children: [
        for (int i = 1; i <= finalSet; i++)
          Padding(
            padding: const EdgeInsets.all(8.0),
            child: Column(
              crossAxisAlignment: CrossAxisAlignment.start,
              children: [
                Text(setsByRound[i]![0].fullRoundText!, style: theme.textTheme.labelLarge),
                for (int j = 0; j < setsByRound[i]!.length; j++)
                  Stack(
                    children: [
                      Padding(
                      ...
                      Padding(
                        ...
                        child: Container(
                            ...
                            child: Text(setsByRound[i]![j].identifier!, style: theme.textTheme.labelLarge!.merge(TextStyle(color: Colors.white)))),
                      ),
                    ],
                  )
              ],

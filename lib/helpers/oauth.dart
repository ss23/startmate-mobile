...
class OAuthToken extends _$OAuthToken {
    ...
    if (exists) {
      ...
      if (credentials.isExpired && credentials.canRefresh) {
        ...
        try {
          ...
          _log.warning("Failed to refresh credentials, despite having a valid refresh token");
          _log.warning(e);
      ...
      if (!credentials.isExpired) {
        ...
        var client = oauth2.Client(credentials, identifier: identifier, secret: secret);
        return client.credentials.accessToken;
      }
  ...
  authenticate() async {
    ...
    _grant = oauth2.AuthorizationCodeGrant(identifier, authorizationEndpoint, tokenEndpoint, secret: secret, onCredentialsRefreshed: (p0) => saveNewCredentials(p0));

    // A URL on the authorization server (authorizationEndpoint with some additional
    // query parameters). Scopes and state can optionally be passed into this method.
    var authorizationUrl = _grant!.getAuthorizationUrl(redirectUrl, scopes: ['user.identity']);
  ...
  finish(Uri responseUrl) async {
    ...
    var client = await _grant!.handleAuthorizationResponse(responseUrl.queryParameters);
